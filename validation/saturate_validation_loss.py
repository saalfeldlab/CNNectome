...
class Clefts:
    def __init__(self, to_be_evaluated, gt, inverted_mask, scale=200.):
        self.scale = scale
        test_clefts = to_be_evaluated
        truth_clefts = gt
        self.truth_clefts_invalid = np.logical_or(truth_clefts == 0xfffffffffffffffe, truth_clefts ==
                                                  0xfffffffffffffffd)
        #print(np.sum(truth_clefts == 0xffffffffffffffff), np.sum(self.truth_clefts_invalid), np.sum(inverted_mask))
        self.truth_clefts_mask = np.logical_or(np.logical_or(truth_clefts == 0xffffffffffffffff,
                                                            self.truth_clefts_invalid),
                                 inverted_mask)
        ...
        self.test_clefts_mask = np.logical_or(np.logical_or(test_clefts == 0,self.truth_clefts_invalid), inverted_mask)
        #print(np.sum(self.test_clefts_mask), np.sum(np.logical_not(self.test_clefts_mask)))
        self.test_clefts_edt = ndimage.distance_transform_edt(self.test_clefts_mask, sampling=(40., 4., 4.))
        self.test_clefts_edt = np.tanh(self.test_clefts_edt/self.scale)
        self.truth_clefts_edt = ndimage.distance_transform_edt(self.truth_clefts_mask, sampling=(40., 4., 4.))
...
def run_evaluation(experiment_name, scale=200):
    ...
    for sample in ['A','B', 'C']:
        ...
        truth_path = '/groups/saalfeld/saalfeldlab/larissa/data/cremi-2017/sample_{0:}_20160501.aligned.uncompressed.hdf'.format(sample)
        ...
        for iteration in iterations:
            validation_json ='/nrs/saalfeld/heinrichl/synapses/miccai_experiments/{0:}/{1:}.n5/it_{' \
                             '2:}/validation_saturated_s{3:}.json'.format(experiment_name, sample, iteration,scale)
            if os.path.exists(validation_json):
                continue
            else:
                test_path = '/nrs/saalfeld/heinrichl/synapses/miccai_experiments/{0:}/{1:}.n5'.format(experiment_name,
                                                                                                      sample)
                ...
                print("VALIDATION")
                print("quick shape test", test_val.shape, truth_val.shape, mask_val.shape)
                cleft_eval = Clefts(test_val, truth_val, np.logical_not(mask_val), scale=scale)
                v_res = dict()
                ...
                print('fn', v_res['v_fn'],  v_res['v_df'], 'fp' ,v_res['v_fp'], v_res['v_dgt'])
