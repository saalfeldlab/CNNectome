...
from cremi.evaluation import NeuronIds, Clefts, SynapticPartners, SynapticPartnersMultRec, SynapticPartnersMultRecGt
import logging
import sys
...
def evaluate_multrecgt(test, truth, add_in_file=False):
    ...
    if add_in_file:
        fscore, precision, recall, fp, fn, filtered_matches, annot = synaptic_partners_eval.fscore(test.read_annotations(),
                                                                      truth.read_annotations(), truth.read_neuron_ids(

        ), all_stats=True, add_in_file=add_in_file)
