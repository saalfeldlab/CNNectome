...
def test(labels):
    ...
    for k, (inp, vs) in enumerate(zip(scnet.input_shapes, scnet.voxel_sizes)):
        input_arrays[net_io_names['raw_{0}'.format(vs[0])]] = np.random.random(inp.astype(np.int))
    for l in labels:
        input_arrays[net_io_names['gt_' + l.labelname]] = np.random.random(scnet.output_shapes[0].astype(np.int))
        if l.scale_loss or l.scale_key is not None:
            input_arrays[net_io_names['w_'+l.labelname]] = np.random.random(scnet.output_shapes[0].astype(np.int))
            requested_outputs[l.labelname]=net_io_names[l.labelname]
        input_arrays[net_io_names['mask_'+l.labelname]] = np.random.random(scnet.output_shapes[0].astype(np.int))
