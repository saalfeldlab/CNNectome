...
def filter(h5filepath, csv_file_src, csv_file_tgt=None, cleft_ds_name='syncleft_dist_thr0.0_cc'):
    logging.info("Filtering clefts in {0:}/{1:} with {2:}".format(h5filepath, cleft_ds_name, csv_file_src))
    ...
    for cleft_id in cleft_list_unmatched:
        ...
        cleft_center = (40.*cleft_coords[0][int(len(cleft_coords[0])/2.)],
                        4.*cleft_coords[1][int(len(cleft_coords[1])/2.)],
                        4.*cleft_coords[2][int(len(cleft_coords[2])/2.)])
...
def main():
    for sample in ['A', 'B', 'C']:
        h5filepath = '/groups/saalfeld/home/heinrichl/sample_{0:}_padded_20181003.aligned.hdf'.format(sample)
        csv_file = '/groups/saalfeld/home/heinrichl/sample_{0:}_partners.csv'.format(sample)
        csv_file_tgt = '/groups/saalfeld/home/heinrichl/sample_{0:}_unmatched_clefts.csv'.format(sample)
