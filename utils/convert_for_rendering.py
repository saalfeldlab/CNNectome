...
def add_ds_combined(
    target, name, data, all_data, labelid, chunks, resolution, offset, thr=127
):
...
def main():
    ...
    for labelname, labelid, thr in labels_combined_all:
        _, all_data = add_ds_combined(
            target,
            "volumes/labels/{0:}".format(labelname),
            orig[labelname],
            all_data,
            labelid,
            (128, 128, 128),
