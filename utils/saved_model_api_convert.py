...
def simple_save(graph, export_dir, inputs, outputs, legacy_init_op=None):
    ...
    with tf.Session(graph=graph) as session:
        saver = tf.train.import_meta_graph(self.graph_name+'.meta', clear_devices=True)
        saver.restore(self.session, self.graph_name)
        b = tf.saved_model.builder.SavedModelBuilder(export_dir)
        pred_sig = (tf.saved_model.signature_def_utils.build_signature_def(inputs=inputs, outputs=outputs,
        ...
        b.add_meta_graph_and_variables(
            ...
            signature_def_map={tf.saved_model.signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY: pred_sig},
...
class Converter(object):
    ...
    def read_meta_graph(self):
        ...
        with open(os.path.join(os.path.dirname(self.graph_name),'net_io_names.json'), 'r') as f:
    ...
    def convert(self, target):
        builder = simple_save(self.graph_name, export_dir=target, inputs=inputs, outputs=outputs)
        tensor_info_raw = tf.saved_model.utils.build_tensor_info(self.graph.as_graph_element(str(self.net_io_names['raw'])))
        tensor_info_dist = tf.saved_model.utils.build_tensor_info(self.graph.as_graph_element(str(self.net_io_names['dist'])))
