...
def add_container(target, name, shape, chunks=(128,128,128), dtype='uint64', resolution=(4,4,4), offset=(0,0,0)):
    if name not in target:
        ds = target.create_dataset(name, shape=shape, chunks=chunks, dtype=dtype, compression='gzip')
        target[name].attrs['resolution']=resolution
        target[name].attrs['offset'] = offset


def add_data_combined_block(coord, blocksize, target, name, data, all_data, labelid, thr):
    s = tuple(slice(c, min(c+b, maxs), None) for c, b, maxs in zip(coord, blocksize, target[name].shape))
...
def add_labels_blockwise(coord, blocksize, labels, target, orig):
    bs = [min(c + b, maxs)-c for c, b, maxs in zip(coord, blocksize, target['volumes/labels/all'].shape)]

    block_data_all = np.ones(bs, dtype=np.uint64) * TRANSPARENT
    for label in labels:
        print('...{0:}'.format(label.labelname))
        block_data_all = add_data_combined_block(coord, bs, target, 'volumes/labels/{0:}'.format(label.labelname),
                                                    orig[label.labelname], block_data_all, label.targetid,
                                                 thr=label.thr)
...
def main():
    ...
    orig = z5py.File('/nrs/saalfeld/heinrichl/cell/gt122018/setup01/run02/test2_55000.n5',
    ...
    labels.append(Label('mito_membrane', 3, 3,thr=123, scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('mito_DNA', 5, 5, thr=128,scale_loss=False, scale_key=labels[-2].scale_key, data_sources=data_sources))
    labels.append(Label('vesicle', (8, 9),9, thr=133,data_sources=data_sources))
    labels.append(Label('vesicle_membrane', 8, 8,thr=123,scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('MVB', (10, 11), 11, thr=133,data_sources=data_sources))
    labels.append(Label('MVB_membrane', 10, 10,thr=123,scale_loss=False, scale_key=labels[-1].scale_key, \
                                                                         data_sources=data_sources))
    labels.append(Label('lysosome', (12, 13), 13,thr=133,data_sources=data_sources))
    labels.append(Label('lysosome_membrane', 12, 12,thr=123,scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('LD', (14, 15),15, thr=133,data_sources=data_sources))
    labels.append(Label('LD_membrane', 14,14, thr=123,scale_loss=False, scale_key=labels[-1].scale_key, \
                                                                        data_sources=data_sources))
    labels.append(Label('er', (16, 17, 18, 19, 20, 21), 17,thr=133, data_sources=data_sources))
    labels.append(Label('er_membrane', (16, 18, 20), 16,thr=123,scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('ERES', (18, 19), 19,thr=133,data_sources=data_sources))
    labels.append(Label('ERES_membrane', 18, 18,thr=123,scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('nucleus', (20, 21, 24, 25),25, thr=133,data_sources=data_sources))
    labels.append(Label('NE', (20, 21), 21,thr=133,scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('NE_membrane', 20, 20,thr=123,scale_loss=False, scale_key=labels[-1].scale_key, \
                                                                        data_sources=data_sources))
    labels.append(Label('chromatin', 24, 24,thr=128,scale_loss=False, scale_key=labels[-1].scale_key,
                        data_sources=data_sources))
    labels.append(Label('nucleoplasm', 25, 25, thr=128,scale_loss=False, scale_key=labels[-1].scale_key,
    ...
    res = [4.0, 4.0, 4.0]
    offset = [0.0, 0.0, 0.0]
    target = z5py.File('/nrs/saalfeld/heinrichl/cell/gt122018/setup01/run02/test2_55000_render_adapthr.n5',
                       use_zarr_format=False)
    ...
    for label in labels:
        add_container(target, 'volumes/labels/{0:}'.format(label.labelname), shape, resolution=res, offset=offset)
    add_container(target, 'volumes/labels/all', shape, resolution=res, offset=offset)
    add_container(target, 'volumes/raw', shape, dtype='uint8', resolution=res, offset=offset)
    ...
    for z in range(0, shape[0], blocksize[0]):
        for y in range(0, shape[1], blocksize[1]):
            for x in range(0, shape[2], blocksize[2]):
                ...
                add_data_asis_block((z, y, x), blocksize, target, 'volumes/raw', orig_raw['volumes/raw'])
