...
class ResultFile:
    def __init__(self,experiment_name, sample, iteration, mode):
        self.mode = mode
        jsonfile = '/nrs/saalfeld/heinrichl/synapses/miccai_experiments/{0:}/{1}.n5/it_{2:}/{3:}.json'.format(
            experiment_name,sample, iteration, mode)
...
class ExpResult:
    ...
    def get_all_it_cs(self, mode, sample=('A', 'B', 'C')):
        ...
        for it in self.all_iterations:
            res_thisit = []
            for s in sample:
                rf = ResultFile(self.experiment_name, s, it, mode)
                res_thisit.append(rf.get_cremi_score())
            res.append(np.mean(res_thisit))
        return res

    def get_all_it_gcs(self, mode, sample=('A', 'B', 'C')):
        if isinstance(sample, str):
            sample = (sample, )
        res = []
        for it in self.all_iterations:
    ...
    def get_all_it_adgt(self, mode, sample=('A', 'B', 'C')):
        if isinstance(sample, str):
            sample = (sample, )
        res = []
        for it in self.all_iterations:
    ...
    def get_all_it_adf(self, mode, sample=('A', 'B', 'C')):
        if isinstance(sample, str):
            sample = (sample, )
        res = []
        for it in self.all_iterations:
...
if __name__ == '__main__':
    ...
    for experiment_name in experiment_names:
        ...
        plt.plot(new_it, new_x, label=er.experiment_name)
