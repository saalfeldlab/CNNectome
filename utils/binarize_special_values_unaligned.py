...
def rewrite_mask(sample):
    ...
    print("raw...", end='')
    # copy raw data
    g.create_dataset('volumes/raw', data=np.array(f['volumes/raw']), chunks=(26, 256, 256))
    ...
    g.create_dataset('annotations/comments/comments', data=f['annotations/comments/comments'],
    ...
    g.create_dataset('annotations/comments/target_ids', data=f['annotations/comments/target_ids'],
    ...
    g.create_dataset('annotations/presynaptic_site/partners', data=f['annotations/presynaptic_site/partners'])
    ...
    g.create_dataset('annotations/ids', data=f['annotations/ids'], chunks=f['annotations/ids'].chunks)
    for k, v in f['annotations/ids'].attrs.iteritems():
        g['annotations/ids'].attrs.create(k, v)

    g.create_dataset('annotations/locations', data=f['annotations/locations'], chunks=f['annotations/locations'].chunks)
    for k, v in f['annotations/locations'].attrs.iteritems():
        g['annotations/locations'].attrs.create(k, v)

    g.create_dataset('annotations/types', data=f['annotations/types'], chunks=f['annotations/types'].chunks)
