...
def rewrite_mask(sample):
    ...
    f = h5py.File('/groups/saalfeld/saalfeldlab/larissa/data/cremi-2017/sample_{'
                  '0:}_20160501.aligned.uncompressed.hdf'.format(sample), 'r')
    g = h5py.File('/groups/saalfeld/saalfeldlab/larissa/data/cremi-2017/sample_{'
                  '0:}_cleftsorig_withvalidation.hdf'.format(sample), 'w')
    ...
    mask = np.logical_not(np.logical_or(clefts_orig == 0xffffffffffffffff-2, clefts_orig == 0xffffffffffffffff-1))
    print("Mask has dtype", mask.dtype)
    g.create_dataset('volumes/masks/groundtruth', data=mask, dtype='float32',chunks=(26,256,256))
    ...
    g.create_dataset('volumes/masks/training', data=training_mask, dtype='float32', chunks=(26,256,256))
    for k, v in f['volumes/masks/training'].attrs.iteritems():
    ...
    g.create_dataset('volumes/masks/validation', data=validation_mask, dtype='float32', chunks=(26,256,256))
