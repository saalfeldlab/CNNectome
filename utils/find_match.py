...
def match_one_block_ds(large_ds, pattern_arr, candidate, axis_orders=(0, 1, 2), verbose=True):
    pattern_shape_zyx = pattern_arr.shape
    for ao in axis_orders:
        pattern_shape = tuple(pattern_shape_zyx[ao_zyx] for ao_zyx in ao)

    #print(candidate)
        if np.array([candidate[k]+pattern_shape[k] <= large_ds.shape[k] for k in range(len(candidate))]).all():
            candidate_block = np.zeros(pattern_shape, dtype=np.uint8)
            slice_from_large = (slice(candidate[0], candidate[0] + pattern_shape[0], None),
        ...
        #print(candidate_block.shape, candidate_block.shape)
            ...
            if (candidate_block == pattern_arr.transpose(ao)).all():
                if verbose:
                    print()
                    print(tuple(str(candidate[k])+'/'+str(large_ds.shape[k]) for k, ao_zyx in enumerate(ao)))
...
def match_one_block_arr(arr_sl, pattern, candidate, axis_orders=(0, 1, 2)):
    ...
    for ao in axis_orders:
        ...
        if np.array([candidate[k]+pattern_shape[k] <= arr_sl.shape[k] for k in [1, 2]]).all():

            slice_from_large = (slice(0, 1, None),
                            slice(candidate[1], candidate[1]+pattern_shape[1], None),
                            slice(candidate[2], candidate[2]+pattern_shape[2], None))
            pattern_sl = (slice(0, 1, None), slice(None, None, None), slice(None, None, None))
...
def test():
    ...
    crop_slice1 = (slice(crop_loc1[0], crop_loc1[0]+crop_size1[0], None), 
    ...
    crop_slice2 = (slice(crop_loc2[0], crop_loc2[0]+crop_size2[0], None),
    ...
    crop_slice3 = (slice(crop_loc3[0], crop_loc3[0]+crop_size3[0], None),
    ...
    pattern_files = [(os.path.expanduser('~/tmp/test_crop1.h5'), 'main'),
