...
from unet import conv_pass, downsample, upsample, crop_zyx
...
def srunet(
    ...
    with tf.name_scope("lev%i"%layer):
        ...
        f_sr_left, voxel_size_up = upsample(
            f_left,
            upscaling_factors,
            num_fmaps,
            activation=activation,
            name='unet_layer_%i_sr_left'%layer,
            fov=fov,
            voxel_size=voxel_size,
            prefix=prefix
        )
        print(prefix + "f_sr_left: " + str(f_sr_left.shape))
        ...
        if bottom_layer:
            ...
            return f_sr_left, fov, voxel_size_up
        ...
        g_out, fov, voxel_size = srunet(
            ...
            upscaling_factors=upscaling_factors,
            activation=activation,
            layer=layer+1,
            fov=fov,
            voxel_size=voxel_size)
        ...
        g_out_upsampled, voxel_size = upsample(
            ...
            voxel_size=voxel_size,
            prefix=prefix)
        assert voxel_size == voxel_size_up
        print(prefix + "g_out_upsampled: " + str(g_out_upsampled.shape))

        # copy-crop
        f_left_cropped = crop_zyx(f_sr_left, g_out_upsampled.get_shape().as_list())
        ...
        f_out,  fov = conv_pass(
            ...
            fov=fov,
            voxel_size=voxel_size_up,
            prefix=prefix
            )

        print(prefix + "f_out: " + str(f_out.shape))

    return f_out, fov, voxel_size_up
...
#
#    model, ll_fov, vx = srunet(raw_batched,
