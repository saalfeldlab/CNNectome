from networks import scale_net, ops3d
...
def make_net(labels, added_steps, mode='train', loss_name='loss_total'):
    ...
    if mode.lower() == 'train' or mode.lower() == 'training':
        ...
        opt = tf.train.AdamOptimizer(learning_rate=0.5e-4, beta1=0.95, beta2=0.999, epsilon=1e-8)
    ...
    return scnet
def make_any_scale_net(serial_unet_list, labels, added_steps, mode='train', loss_name='loss_total'):
    # input_voxel_size=(
    # 36,36,36))
    ...
    scnet = scale_net.ScaleNet(serial_unet_list, input_size_actual, name='scnet_'+mode)
    inputs = []
    names = dict()
    for k, (inp, vs) in enumerate(zip(scnet.input_shapes, scnet.voxel_sizes)):
        raw = tf.placeholder(tf.float32, shape=inp)
...
def make_mini_net(labels, added_steps, mode='train', loss_name='loss_total'):
    unet0 = scale_net.SerialUNet([12, 12*6,], [48, 12*6],
                                 [(2, 2, 2)],
                                 [[(3, 3, 3)], [(3, 3, 3)]],
                                 [[(3, 3, 3)] ], input_voxel_size=(1, 1, 1))
    unet1 = scale_net.SerialUNet([12, 12 * 6], [48, 12 * 6],
                                 [(2, 2, 2),],
                                 [[(3, 3, 3)], [(3, 3, 3)]],
                                 [[(3, 3, 3)]], input_voxel_size=(2, 2, 2))
