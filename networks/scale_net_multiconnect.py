...
class ScaleNet(object):
    def __init__(self, list_of_serialunets, input_shape, name='scnet'):
        ...
        bottom_shape = self.list_of_serialunets[0].get_bottom_shape_from_input_shape(input_shape)
        output_shape = self.list_of_serialunets[0].get_output_shape_from_input_shape(input_shape)
        ...
        for lv in range(len(self.list_of_serialunets))[1:]:
            ...
            padding_left += (self.voxel_sizes[lv]/self.voxel_sizes[lv-1]) * ((self.output_shapes[lv] -
                                                                              self.bottom_shapes[lv-1]) // 2)
            padding_right += (self.voxel_sizes[lv]/self.voxel_sizes[lv-1]) * ((self.output_shapes[lv] -
                                                                               self.bottom_shapes[lv-1] +
                                                                               np.array((1, 1, 1))) // 2)
            padding_left += (self.voxel_sizes[lv]/self.voxel_sizes[lv-1]) * (self.input_shapes[lv] -
                                                                             self.output_shapes[lv])/2
            padding_right += (self.voxel_sizes[lv]/self.voxel_sizes[lv-1]) * (self.input_shapes[lv] -
                                                                              self.output_shapes[lv])/2
    ...
    def build(self, inputs):
        bottom_insert = None
        for serialunet, inp, vox in zip(self.list_of_serialunets[::-1], inputs[::-1], self.voxel_sizes[::-1]):
            print("Building Unet with voxelsize {0:}".format(vox))
            bottom_insert, fov, vs = serialunet.build(inp, fmaps_bottom=bottom_insert,
...
class SerialUNet(object):
    ...
    def is_valid_input_shape(self, input_shape):
        if (input_shape >= self.min_input_shape).all() and ((input_shape-self.min_input_shape) % self.step_valid_shape \
                == 0).all():
            return True
        else:
            return False
...
if __name__ == "__main__":
    unet0 = SerialUNet(36, 2, [(1, 3, 3), (1, 3, 3)],
                       [[(1, 3, 3), (1, 3, 3)], [(1, 3, 3), (1, 3, 3)], [(3, 3, 3), (3, 3, 3)]],
                       [[(1, 3, 3), (1, 3, 3)], [(1, 3, 3), (1, 3, 3)], [(3, 3, 3), (3, 3, 3)]])
    unet1 = SerialUNet(72, 2, [(3, 3, 3), (3, 3, 3)],
    ...
    for inp in comb.input_shapes[1:]:
        ...
        raw_downscaled_batched = tf.reshape(raw_downscaled, (1, 1, ) + tuple(inp.astype(np.int)))
