...
class UNet(object):
    ...
    def __init__(self, num_fmaps_down, num_fmaps_up, downsample_factors, kernel_size_down, kernel_size_up,
                 activation='relu', constant_upsample=False, trans_equivariant=False, input_fov=(1, 1, 1), \
    ...
    def compute_minimal_shapes(self):
        ...
        min_bottom_right = [(1., 1., 1.)]*(len(self.downsample_factors)+1)
        for lv in range(len(self.downsample_factors)):
            kernels = np.copy(self.kernel_size_up[lv])

            total_pad = np.sum([np.array(k) - np.array((1., 1., 1.)) for k in kernels], axis=0) # padding needed for
            # convolutions on upsamling side on level lv
            if self.trans_equivariant:
                total_pad = np.ceil(total_pad/np.prod(self.downsample_factors[lv:], axis=0, dtype=np.float))*np.prod(
        ...
        for lv in range(len(self.kernel_size_down))[::-1]:
            ...
            total_pad = np.sum([np.array(k) - np.array((1., 1., 1.)) for k in kernels], axis=0)
    ...
    def build(self,
        ...
        with tf.name_scope("lev%i" % layer):
            ...
            if self.trans_equivariant:
                ...
                g_out_upsampled = ops3d.crop_to_factor(g_out_upsampled, factor=factor_product,
