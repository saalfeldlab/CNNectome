...
def match_approx(large_ds, patterns_ds, axis_orders, thr=0.7):
    ...
    for p in patterns:
        for ao in axis_orders:
            pattern_patches.append(
                p[
                    : min(64, p.shape[0]), : min(64, p.shape[1]), : min(64, p.shape[2])
                ].transpose(ao)
            )
        ...
        for y_in_ds in range(64, large_ds.shape[1] - 64, large_ds.chunks[1]):
            ...
            for x_in_ds in range(64, large_ds.shape[2], large_ds.chunks[2]):
                tile_slice = (
                    slice(
                        z_in_ds,
                        min(z_in_ds + large_ds.chunks[0], large_ds.shape[0]),
                        None,
                    ),
                    slice(
                        y_in_ds,
                        min(y_in_ds + large_ds.chunks[1], large_ds.shape[1]),
                        None,
                    ),
                    slice(
                        x_in_ds,
                        min(x_in_ds + large_ds.chunks[2], large_ds.shape[2]),
                        None,
                    ),
                )
...
def main(large_h5, patterns_h5, axis_orders=[(0, 1, 2)]):
    ...
    finds = match_approx(large_ds, patterns_ds, axis_orders=axis_orders)
...
if __name__ == "__main__":
    ...
    finds = main(
        large_ds_file, pattern_files, axis_orders=[(0, 1, 2)]
