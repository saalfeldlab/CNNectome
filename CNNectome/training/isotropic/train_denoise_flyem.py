...
def batch_generator(
    ...
    for aug in augmentations:
        ...
        #     pipeline += GammaAugment([ArrayKey("RAW_INPUT"), ArrayKey("RAW_TARGET")], 0.5, 2.)
            ...
            raise ValueError("")
    pipeline += corditea.Multiply((ArrayKey("RAW_INPUT"), ArrayKey("DATA_MASK")), ArrayKey("RAW_INPUT"),
...
def evaluate_metric(eval_iterations: int,
                    metric: str,
                    ...
                    gamma_range: typing.Tuple[float, float] = (0.75, 4 / 3.),
                    impulse_noise_prob: float = 0.05,
                    prob_missing: float = 0.05,
                    prob_low_contrast: float = 0.05,
                    contrast_scale: float = 0.1,
                    voxel_size: typing.Union[Coordinate, typing.Tuple[int, int, int]] = Coordinate((4, 4, 4))
                    ) -> typing.List[float]:
    pipeline, request, array_specs_pred = batch_generator(data_path,
                                                          raw_dataset,
                                                          input_shape,
                                                          output_shape,
                                                          augmentations,
    ...
    if net_name is not None:
        pipeline + PreCache(cache_size=cache_size,
                            num_workers=num_workers)
        if metric_in_graph(metric, net_name):
            net_io_names, _, inputs, outputs = network_setup(net_name, load_checkpoint=False, mode="forward")
            ...
            request.add(ak_metric, voxel_size, voxel_size=voxel_size)  # todo: potential issue with shape here
        else:
            net_io_names, _, inputs, outputs = network_setup(net_name, load_checkpoint=False, mode="inference")
            graph = "{net_name:}_inference.meta".format(net_name=net_name)
        if iteration is not None:
            pipeline += Predict(
                "{net_name:}_training_checkpoint_{it:}".format(net_name=net_name, it=iteration),
                inputs=inputs,
                outputs=outputs,
                array_specs=array_specs_pred,
                graph=graph)
    ...
    with build(pipeline) as pp:
        for i in range(eval_iterations):
            ...
            if net_name is not None and metric_in_graph(metric, net_name):
                ...
                res = compute_metric(b.arrays[ArrayKey(ak_out)].data, b.arrays[ArrayKey(ak_ref)].data, metric)
            results.append(res)
            time_it = time.time() - start_it
            logging.info("it{0:}: {1:}".format(i+1, time_it))
    return results
