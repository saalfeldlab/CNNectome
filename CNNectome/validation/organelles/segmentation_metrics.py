...
class EvaluatorError(Exception):
    def __init__(self, array_name, values):
        message = (
            f"Array {array_name:} should only contain values 0, 1 but found {values:}"
        )
...
def best(argument):
    switcher = {-1: np.nanargmax, 1: np.nanargmin}
...
class Evaluator(object):
    def __init__(
        ...
        self.cremieval = cremi_scores.CremiEvaluator(
            self.truth,
            self.test,
            sampling=resolution,
    ...
    def voi(self):
        if self.truth_empty or self.test_empty:
            ...
            if self.mask is not None:
                test_voi = (
                    self.test + self.mask
                )  # add 1 in valid area, keep values of 0 in invalid area
                truth_voi = self.truth + self.mask
            else:
                test_voi = self.test + 1
                truth_voi = self.truth + 1
            voi_split, voi_merge = cremi.evaluation.voi(
                test_voi, truth_voi, ignore_groundtruth=[0], ignore_reconstruction=[0]
            )
