...
def get_best_manual(dataset, labelname, setup=None, s1=False, training_version="v0003.2"):
    csv_folder = os.path.join(config_loader.get_config()["organelles"]["evaluation_path"], training_version, "manual")
    if setup is None:
...
def query_score(cropno, labelname, threshold=127, setup=None, s1=False, clip_distance=200,
...
def get_differences(cropno, metrics, domain="setup", threshold=127, clip_distance=200, tol_distance=40,
                    training_version="v0003.2", gt_version="v0003"):
    ...
    for manual_result in all_manual:
        ...
        for best_by_metric in metrics + ["manual"]:
            ...
            for eval_metric in metrics:
                ...
                entry = "{eval_metric:}_by_{best_by_metric:}".format(eval_metric=eval_metric,
                                                                     best_by_metric=best_by_metric)
                if best_by_metric == "manual":
                    manual_result[entry] = query_score(cropno, manual_result["labelname"],
                                                       setup=query_setup, s1=manual_result["s1"],
                                                       threshold=threshold, clip_distance=clip_distance,
                                                       tol_distance=tol_distance, training_version=training_version,
                                                       gt_version=gt_version)[eval_metric]
                else:
                    specific_params = filter_params(metric_params, best_by_metric)
                    best_result = get_best_automatic(db, cropno, manual_result["labelname"], best_by_metric,
                                                     specific_params,
