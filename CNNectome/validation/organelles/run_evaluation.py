...
def add_constant(seg, resolution, label):
    inner_distance = scipy.ndimage.distance_transform_edt(
        scipy.ndimage.morphology.binary_erosion(
            seg,
            border_value=1,
            structure=scipy.ndimage.generate_binary_structure(seg.ndim, seg.ndim),
        ),
        sampling=resolution,
    )
    outer_distance = scipy.ndimage.distance_transform_edt(
        np.logical_not(seg), sampling=resolution
    )
    ...
    for tsp in config_loader.get_config()["organelles"]["training_setups_paths"].split(
        ...
        if os.path.exists(setup_path):
            pred_path = os.path.join(
                setup_path, crop_utils.get_data_path(crop, s1).format(iteration)
            )
            return pred_path
    raise FileNotFoundError("Have not found location for setup {0:}".format(setup))
...
def run_validation(
    ...
    if set(results.keys()) != set(metrics):
        gt_empty = not any(
            l in crop_utils.get_label_ids_by_category(crop, "present_annotated")
            for l in label.labelid
        )
        offset, shape = crop_utils.get_offset_and_shape_from_crop(
            crop, gt_version=gt_version
        )
        if label == "cytosol":
            assert setup == "setup01" or setup == "setup02"
            test_binary, resolution = reconstruct_cytosol_prediction(
                pred_path, offset, shape, thr=threshold
            )
        else:
            if refined:
                refined_prediction, resolution = read_prediction(
                    pred_path, pred_ds, offset, shape
                )
                test_binary = make_binary(refined_prediction)
            else:
                prediction, resolution = read_prediction(
                    pred_path, pred_ds, offset, shape
                )
...
def main(alt_args=None):
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="label for which to evaluate prediction, choices: "
        + ", ".join(list(hierarchy.keys())),
    )
    parser.add_argument(
        "--crop",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="v0003.2",
        help="Version of training from which to " "evaluate setup.",
    )
    parser.add_argument(
        "--gt_version",
        type=str,
        default="v0003",
        help="Version of groundtruth to use for evaluation.",
    )
    parser.add_argument(
        "--save", action="store_true", help="save to database and csv file"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        "--refined", action="store_true", help="use refined predictions"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    num_validations = max(
        len(list(always_iterable(args.setup))),
        len(list(always_iterable(args.iteration))),
        len(list(always_iterable(args.label))),
        len(list(always_iterable(args.pred_path))),
        len(list(always_iterable(args.pred_ds))),
        len(list(always_iterable(args.threshold))),
        1,
    )
    iterator = itertools.product(
        zip(
            range(num_validations),
    ...
    for (valno, setup, iteration, label, pred_path, pred_ds, thr), crop in iterator:
        ...
        if not os.path.exists(os.path.join(pred_path, "attributes.json")):
            raise RuntimeError(
                "N5 is incompatible with zarr due to missing attributes files. Consider running"
        ...
        if label is None:
            ...
            for ll in labels:
                if ll not in crop_utils.get_all_annotated_labelnames(crop):
                    raise ValueError(
                        "Label {0:} not annotated in crop {1:}".format(
                            ll, crop["number"]
                        )
                    )
        for ll in labels:
            ...
            if setup is None:
                ...
                if this_setup is not None:
                    ...
                    this_setup = setup
            if not args.refined and pred_path != construct_pred_path(
                this_setup, iter, crop, args.s1, training_version=args.training_version
            ):
                warnings.warn(
    ...
    tabs = [
        (
            ...
            t,
            m,
            filter_params(metric_params, m),
        )
        for (pp, d, s, i, ll, c, r_ds, parent, p_id, t), m in itertools.product(
            validations, metric
        )
    ]
    print(
    ...
    if not args.dry_run:
        ...
        tabs = [
            ...
            for (pp, d, s, i, ll, c, r_ds, parent, p_id, t, r) in validations
            for m, v in r.items()
        ]
        print(
            tabulate.tabulate(
