...
def prepare_cell_inference(n_jobs, raw_data_path, dataset_id, sigma, raw_ds, setup_path, output_path, factor,
    ...
    if not finish_interrupted:
        ...
        for dstk in dataset_target_keys:
            if dstk not in f:
                ds = f.empty(name=dstk, shape=full_shape_vc_output, compressor=numcodecs.GZip(6),
        ...
        shapes_file = os.path.join(setup_path, "shapes_steps_{0:}x{1:}x{2:}.json".format(*size))
...
def single_job_inference(job_no, raw_data_path, sigma, raw_ds, setup_path, output_path=None, factor=None,
    ...
    run_inference_zarr_multi_crop(
        ...
        input_resolutions=[tuple(voxel_size), ],
        target_resolutions=[tuple(voxel_size), ] * len(dataset_target_keys),
...
if __name__ == "__main__":
    ...
    #                 setup_path, factor=factor, min_sc=min_sc, max_sc=max_sc)
        ...
        single_job_inference(n_job, raw_data_path, sigma, raw_ds,
