...
def plot_s1_vs_sub(db_username, db_password, metric, cropno, cropnames, tol_distance=40, clip_distance=200, 
                 threshold=127):
    ...
    for result in all_results:
        ...
        result["label_subsampled"] = short_names[results["label_subsampled"]]
        labels.append(result["label_s1"])
    labels = list(set(labels))
    lables = sort_generic(labels)
    ...
    for m, label in enumerate(labels):
        ...
        for result in all_Results:
            if result["label_subsampled"] == label:
                ...
                x_values.append(mid + off)
                y_values.append(float(results["value_s1"]))
    ...
    if save:
        ...
        plt.savefig("/groups/cosem/cosem/computational_evaluation/v0003.2/comparisons/subsampled_vs_averaged_{0:}_{"
                    "1:}.{2:}".format(metric, transparent, filetype), transparent=transparent)
    plt.show()


def sort_generic(labels, sorting_metric = "dice"):
...
def plot_datasets(metric, add_manual=False, filetype="svg", transparent=False, save=False):
    matplotlib.rcParams['font.sans-serif'] = "Arial"
    matplotlib.rcParams['font.family'] = "sans-serif"
    if metric == "dice":
    ...
    if save:
        plt.subplots_adjust(left=0,bottom=0,top=1,right=1, hspace=0.15)
        plt.savefig(os.path.join(csv_folder, "comparisons/datasets_{0:}_{1:}.{2:}".format(metric, transparent, filetype)),
                    transparent=transparent)
    plt.show()


def plot_4nm_vs_8nm(metric, filetype="svg", transparent=False, save=False):
...
def plot_rawvsrefined_single(metric, filetype="svg", transparent=False, save=False):
    matplotlib.rcParams['font.sans-serif'] = "Arial"
    matplotlib.rcParams['font.family'] = "sans-serif"
...
def plot_rawvsrefined(metrics,  filetype="svg", transparent=False, save=False):
    if len(metrics) == 1:
...
def plot_all_vs_common_vs_single(metric, filetype="svg", transparent=False, save=False):
    matplotlib.rcParams['font.sans-serif'] = "Arial"
    matplotlib.rcParams['font.family'] = "sans-serif"
    if metric == "dice":
...
def plot_metric_comparison_by_label(metric1, metric2, filetype="svg", transparent=False, save=False):

    matplotlib.rcParams['font.sans-serif'] = "Arial"
    matplotlib.rcParams['font.family'] = "sans-serif"

    def assemble_results(metric1, metric2):
        results = dict()
        ...
        for row in reader_per_setup_validation:
            ...
            for k, res in enumerate(results[row["crop_{0:}".format(metric1)]][row["label_{0:}".format(metric1)]]["test"]):
                if ((results[row["crop_{0:}".format(metric1)]][row["label_{0:}".format(metric1)]]["validation"][k] is None)
                    ...
                    break
        return results
    results = assemble_results(metric1, metric2)
    results_with_manual = assemble_results("dice", "manual")
    labels = []
    for crop, value_by_crop in results.items():
    ...
    for ax in axs:
        ...
        ax.spines["right"].set_visible(False)
        ax.spines["left"].set_visible(False)
    crops_no_to_cell = {"113": "jrc_hela-2", "111": "jrc_hela-3", "112": "jrc_jurkat-1", "110": "jrc_macrophage-2"}
    colors = {"111": "#7570b3",  # lavender
              "113": "#66a61e",  # green
              "110": "#e7298a",  # pink
              "112": "#a6761d"   # ockery
        ...
        for m, label in enumerate(labels):
            ...
            for val, test, off in zip(vals, tests, offsets):
                ...
                ax.scatter(mids[m]+off+centering_shift, metric1_value_validation, c=colors[crop], s=fs*ratio,
                           marker=sym_val)
                ax.scatter(mids[m]+off+centering_shift, metric2_value, c=colors[crop], s=fs*ratio, marker=sym_manual)
    ...
    if save:
        ...
        plt.savefig(os.path.join(csv_folder, "comparisons/metrics_by_label_{0:}vs{1:}_{2:}.{3:}".format(metric1,metric2,
                                                                                                    transparent,
                                                                                                    filetype)),
                transparent=transparent)
    plt.show()
...
def plot_metric_comparison(metric1, metric2, filetype="svg", transparent=False, label=None, save=False):
    matplotlib.rcParams['font.sans-serif'] = "Arial"
    matplotlib.rcParams['font.family'] = "sans-serif"
    def stems(ax, x_values, y_values, colors, marker):
        ax.scatter(x_values, y_values, c=colors, s=fs*ratio, marker=marker)
        # for xx, yy, cc in zip(x_values, y_values, colors):
        #     ax.plot([xx,xx], [0,yy], color=cc, linewidth=.5)
    ...
    stems(ax, x_values_validation, y_values_validation, cols_validation)
...
def main():
    ...
    parser.add_argument("--filetype", type=str, default="svg")
    ...
    if args.plot == "s1_vs_sub":
        plot_s1_vs_sub(args.db_username, args.db_password, args.metric, args.cropno, args.cropnames, save=args.save)
