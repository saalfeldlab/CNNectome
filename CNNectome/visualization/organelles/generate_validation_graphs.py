...
def plot_val_all_setups(
    db: MongoCosemDB, path: str, threshold: int = 127, filetype: str = "pdf"
) -> None:
    """
...
def plot_val_all_labels(
    db: MongoCosemDB, setup: str, path: str, threshold: int = 127, filetype: str = "pdf"
):
    """
    ...
    for lbl in labels:
        ...
        if any(in_crop):
            file = os.path.join(
                path,
                "{label:}_{setup:}.{filetype:}".format(
                    label=lbl.labelname, setup=setup, filetype=filetype
                ),
            )
...
def main() -> None:
    ...
    all_parser.add_argument(
        "--path", type=str, default=".", help="Path to save validation graphs to."
    )
    all_parser.add_argument(
        "--filetype", type=str, default="pdf", help="Filetype for validation plots."
    )
    all_parser.add_argument(
        "--training_version",
        type=str,
    ...
    setup_parser = parser.add_parser(
        "all_labels", help="Validation graphs for all labels of a specific setup."
    )
    setup_parser.add_argument(
        "setup", type=str, help="Setup to make validation graphs for."
    )
    setup_parser.add_argument(
        "--path", type=str, default="Path to save validation graphs to."
    )
    setup_parser.add_argument(
        "--filetype", type=str, default="pdf", help="Filetype for validation plots."
    )
    ...
    single_parser.add_argument(
        "setup", type=str, help="Setup to make validation graph for."
    )
    single_parser.add_argument(
        "label", type=str, help="Label to make validation graph for."
    )
    single_parser.add_argument(
        "--file", type=str, help="File to save validation graph to. (Full path)"
    )
    single_parser.add_argument(
        "--threshold",
        type=int,
    ...
    if args.script == "all_setups":
        plot_val_all_setups(
            db, args.path, threshold=args.threshold, filetype=args.filetype
        )
    elif args.script == "all_labels":
        plot_val_all_labels(
            db, args.setup, args.path, threshold=args.threshold, filetype=args.filetype
        )
    else:
