from CNNectome.networks import ops3d_tf2 as ops3d
import numpy as np
...
def make_net(unet, added_steps, loss_name="loss_total", padding="valid", mode="train"):
    ...
    raw = tf.keras.layers.InputLayer(
        dtype=tf.float32, input_shape=tuple(input_size_actual)
    )
    names["raw"] = raw.name
    ...
    U = tf.keras.Model(inputs=raw, outputs=[cleft_dist, pre_dist, post_dist])

    if mode.lower() == "train" or mode.lower() == "training":
        gt_cleft_dist = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        gt_pre_dist = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        gt_post_dist = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        ...
        loss_weights_cleft = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        loss_weights_pre = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        loss_weights_post = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        ...
        cleft_mask = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        pre_mask = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
        post_mask = tf.keras.layers.InputLayer(
            dtype=tf.float32, input_shape=output_shape
        )
