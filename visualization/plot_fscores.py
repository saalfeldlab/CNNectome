...
def plot_param(param_settings, plot_by_field='seg', fields_to_plot='fscore 0'):
        ...
        with open("/groups/saalfeld/home/heinrichl/Downloads/prepost.csv", "r") as f:
            ...
            for row in csv_reader:
                ...
                if found:
                    for field_to_plot in fields_to_plot:
                        try:
                            plot_dict[field_to_plot][row[plot_by_field]].append(row[field_to_plot])
                        except KeyError:
                            plot_dict[field_to_plot][row[plot_by_field]] = [row[field_to_plot]]
    print(plot_dict)
    numplots_per_param_sett = len(fields_to_plot)
    dist = 1./(numplots_per_param_sett+1)
    #fig, ax = plt.subplots()
    plt.ylabel('fscore')
    print(dist)
    ...
    for field_idx, field_to_plot in enumerate(fields_to_plot):
        ...
        for k, i in pd.iteritems():
            ...
            if field_idx == 0:
                plt.plot([(field_idx+1)*dist+x-0.5 for x in
                      xrange(0,len(i))],
                     [float(x) for x in i], 'o', label=k)
            else:
                plt.plot([(field_idx+1)*dist+x-0.5 for x in
                      xrange(0,len(i))],
                     [float(x) for x in i], 'o')
...
param_settings = [{'iterations': 400000, 'pre thr': '-', 'size thr': 50, 'cleft thr': 127, 'sample': 'C',
                   'legend':
                       'baseline'},
                  {'iterations': 400000, 'pre thr': 35, 'size thr': 50, 'cleft thr': 127, 'sample': 'C',
                   'legend': 'params1'},
                  {'iterations': 400000, 'pre thr': 60, 'size thr': 50, 'cleft thr': 127, 'sample': 'C',
                   'legend': 'params2'}
                  ]
