...
def load_result(data_train, augmentation, data_eval, iteration, mode):
    result_json = os.path.join('/nrs/saalfeld/heinrichl/synapses/data_and_augmentations', data_train, augmentation,
                               'evaluation', str(iteration), data_eval, 'cleft.{0:}.json'.format(mode))
    try:
...
def compute_cremi_score(samples, data_train, augmentation, data_eval, iteration, mode):
    ...
    score = 0.
    for s in samples:
        score += 0.5*(result[s]['false positive distance']['mean']+result[s]['false negative distance']['mean'])
    score /= len(samples)
    return score

def compute_cremi_score_err(samples, data_train, augmentation, data_eval, iteration, mode):
    result = load_result(data_train, augmentation, data_eval, iteration, mode)
    if result.isnan():
        return result
    err = 0.
    for s in samples:
        err += 0.25*((result[s]['false positive distance']['std'])**2+(result[s]['false negative distance'][
                                                                        'std'])**2)
    err = err**(-0.5)/len(samples)
    return err

def compute_average_cremi_score(samples, data_train, augmentation, data_eval, iterations, mode):
    res = []
    for i in iterations:
        res.append(compute_cremi_score(samples, data_train, augmentation, data_eval, i, mode))
...
def compute_ratio(samples, data_train=None, augmentation=None, data_eval=None, comparisons=(('lite', 'deluxe'),
                                                                                         ('classic',
                                                                                                    'deluxe'),
                                                               ('lite', 'classic'))):
...
def compute_average_ratio(samples, data_train=None, augmentation=None, data_eval=None, comparisons=(('lite', 'deluxe'),
                                                                                         ('classic', 'deluxe'),
                                                                       ('lite', 'classic'))):
    validation_ratios, validation_min_ratios=compute_ratio(samples, data_train=data_train,augmentation=augmentation,
                                                           data_eval=data_eval, comparisons=comparisons)
    averages = dict()
    stds = dict()
    for comp in comparisons:
...
def plot_cremi_score_by_iteration(samples, data_train, augmentation, data_eval):
    ...
    plt.plot(iterations, training_cremi_scores, ls='--', c=colors[augmentation], label=augmentation + ', training',
             linewidth=0.5)
    plt.plot(iterations[minit_training], training_cremi_scores[minit_training], c=colors[augmentation], marker='o',
             alpha=0.5)
    plt.plot(iterations, validation_cremi_scores, ls='-', c=colors[augmentation], label=augmentation + ', validation')
    plt.plot(iterations[minit_validation], validation_cremi_scores[minit_validation], c=colors[augmentation],
             marker='o')
    plt.annotate('{0:.2f}'.format(validation_cremi_scores[minit_validation]), [iterations[minit_validation],
                 validation_cremi_scores[minit_validation]], [4, -7], textcoords='offset points', color=colors[
        augmentation])
...
def summary_plot(data_train, samples):
    ...
    for sp, de in enumerate(['data2017-aligned', 'data2017-unaligned', 'data2016-aligned', 'data2016-unaligned']):
    ...
    plotfile= os.path.join('/nrs/saalfeld/heinrichl/synapses/data_and_augmentations', trained_on,
                           ''.join(samples)+'.pdf')
...
def all_summary_plots():
    ...
    for dt in ['data2017-aligned', 'data2017-unaligned', 'data2016-aligned', 'data2016-unaligned']:
...
def ratio_summary_plots_augmentations():
    ...
    data_eval  = ['data2017-aligned', 'data2017-unaligned', 'data2016-aligned', 'data2016-unaligned']
    ...
    for de in data_eval:
        for k, dt in enumerate(data_train):
            ...
            for l, comp in enumerate(avgs.iterkeys()):
                plt.errorbar(x, avgs[comp], yerr=stds[comp], fmt=symbols[dt], color=colors[comp], ms=12)
                plt.plot(x, mins[comp], marker=symbols[dt], color=colors[comp], ms=7, alpha=0.5)
                x += d_tiny
                if k == np.ceil(len(data_train)/2.)-1 and l == np.ceil(len(avgs)/2.)-1:
                    xt.append(x+((len(avgs)+1) % 2)*d_tiny/2.)
    ...
    for dt in data_train:
        l = mlines.Line2D([],[], color='black', marker=symbols[dt], ms=12, label='trained on '+ dt)
        leg.append(l)
    l = mlines.Line2D([], [], color='black', marker='o', ms=7, label='ratio of best performance', alpha=0.5)
    leg.append(l)
...
def ratio_summary_plots_dt():
    samples = [ 'A', 'B', 'C']
    #data_train= ['data2017-aligned', 'data2017-unaligned', 'data2016-aligned', 'data2016-unaligned']
    comparisons = (('data2016-aligned', 'data2017-aligned'), ('data2016-unaligned', 'data2017-unaligned'),
                   ('data2017-unaligned', 'data2017-aligned'), ('data2016-unaligned', 'data2016-aligned'))
    ...
    for de in data_eval:
        for k, aug in enumerate(augmentations):
            avgs, stds, mins = compute_average_ratio(samples, data_eval=de, augmentation=aug, comparisons=comparisons)
            ...
            for l, comp in enumerate(comparisons):
                plt.errorbar(x, avgs[comp], yerr=stds[comp], fmt=symbols[comp], color=colors[aug], ms=12)
                ...
                plt.plot(x, mins[comp], marker=symbols[comp], color=colors[aug], ms=7, alpha=0.5)
                if k == np.ceil(len(augmentations)/2.)-1 and l == np.ceil(len(avgs)/2.)-1:
                    xt.append(x + ((len(avgs) + 1) % 2) * d_tiny / 2.)
                x += d_tiny
    ...
    for c in comparisons:
        l = mlines.Line2D([],[], color='black', marker=symbols[c], ms=12, label=c[0] + ' / ' + c[1])
        leg.append(l)
    l = mlines.Line2D([], [], color='black', marker='o', ms=7, label='ratio of best performance', alpha=0.5)
    leg.append(l)
    fig.legend(handles=leg)
...
def absolute_summary_plots():
    ...
    #    'magenta'}
    abb = {'data2017-aligned':'17A', 'data2017-unaligned':'17B', 'data2016-aligned':'16A', 'data2016-unaligned': 'B'}
    ...
    for de in data_eval:
        ...
        for k, aug in enumerate(augmentations):
            for l, dt in enumerate(data_train):
                avg_cremi_score, std_cremi_score, min_cremi_score = compute_average_cremi_score(samples, dt, aug, de,
                                                                                                iterations, 'validation')
                plt.errorbar(x, avg_cremi_score, yerr=std_cremi_score, fmt='o', color=colors[aug], ms=12, linewidth=2)
                ...
                # c.set_figure(fig)
                if k == np.ceil(len(augmentations)/2.)-1 and l == np.ceil(len(data_train)/2.)-1:
                    xt.append(x + ((len(data_train) + 1) % 2) * d_tiny / 2.)
                x += d_tiny
            x += d_small
    plt.xticks(xt, ['eval on ' + de for de in data_eval])
    ...
    for aug in augmentations:
        ...
        leg.append(l)
    l = mlines.Line2D([], [], color='black', marker='o', ms=4, label='best CREMI score', linestyle='None')
    leg.append(l)
    fig.legend(handles=leg)
