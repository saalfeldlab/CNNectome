...
def create_prepost_dt(clefts,labels, voxel_size, cleft_to_presyn_neuron_id,
                      cleft_to_postsyn_neuron_id, bg_value=0, include_cleft=True, normalize_mode='tanh',
                      normalize_args=50):
    ...
    if normalize is not None:
        presyn_distances = normalize(presyn_distances, normalize_mode, normalize_args=normalize_args)
        postsyn_distances = normalize(postsyn_distances, normalize_mode, normalize_args=normalize_args)
...
def main():
    ...
    hf = h5py.File(os.path.join(cremi_dir, 'sample_B_padded_20170424.aligned.0bg.hdf'), 'r')
    ...
    print('data loaded')
    pre_dist, post_dist = create_prepost_dt(clefts, labels, (40,4,4), pre_dict, post_dict)
    print('dist computed')
    hf.close()
