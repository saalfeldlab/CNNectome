...
def build_net(steps=steps_inference, mode='inference'):
    unet = unet_class.UNet([12, 12*6, 12*6**2, 12*6**3],
                           [12, 12*6, 12*6**2, 12*6**3],
                           [(1, 3, 3), (1, 3, 3), (3, 3, 3)],
                           [[(1, 3, 3), (1, 3, 3),(1, 3, 3)], [(1, 3, 3), (1, 3, 3),(3, 3, 3)],
                            [(1, 3, 3), (3, 3, 3), (3, 3, 3)], [(3, 3, 3), (3, 3, 3),(3, 3, 3)]],
                           [[(1, 3, 3), (1, 3, 3), (1, 3, 3)], [(1, 3, 3), (1, 3, 3), (3, 3, 3)],
                            [(1, 3, 3), (3, 3, 3), (3, 3, 3)]],
                           constant_upsample=True, trans_equivariant=False,
                           input_voxel_size=(40, 4, 4), input_fov=(40, 4, 4))
    net, input_shape, output_shape = make_net(unet, steps, mode=mode)
    logging.info("Built {0:} with input shape {1:} and output_shape {2:}".format(net, input_shape, output_shape))
...
def test_memory_consumption(steps=steps_train, mode='train'):
    ...
    for l in ['cleft', 'pre', 'post']:
        if mode.lower() == 'train' or mode.lower() == 'training':
            input_arrays[net_io_names['gt_' + l + '_dist']] = np.random.random(output_shape).astype(np.float32)

            input_arrays[net_io_names['loss_weights_'+l]] = np.random.random(output_shape).astype(
                    np.float32)
            input_arrays[net_io_names[l + '_mask']] = np.random.random(output_shape).astype(np.float32)
...
def train(steps=steps_train):
    ...
    train_until(max_iteration, samples, aug_mode, input_shape, output_shape, cremi_dir, n5_filename_format,
                csv_filename_format, filter_comments, dt_scaling_factor, loss_name, net_name)
