...
def train_until(max_iteration, data_sources, input_shape, \
                output_shape, dt_scaling_factor, loss_name,
    ...
    request.add(ArrayKeys.INTEGRAL_MASK, output_size, voxel_size=voxel_size_output)
    ...
    for label in labels:
        ...
        array_specs_pred[label.pred_dist_key] = ArraySpec(voxel_size=voxel_size_output, interpolatable=True)
        ...
        snapshot_request.add(label.pred_dist_key, output_size, voxel_size=voxel_size_output)
    ...
    for src in data_sources:
        ...
        for subsample_variant in range(8):
            d = datasets.copy()
            d[ArrayKeys.RAW] = 'volumes/subsampled/raw/{0:}/'.format(subsample_variant)
    ...
    train_pipeline = (
        data_stream +
        RandomProvider(tuple(np.repeat([ds.labeled_voxels for ds in data_sources], 8))) +
        gpn.SimpleAugment() +
        gpn.ElasticAugment(voxel_size_output, (100, 100, 100), (10., 10., 10.), (0, math.pi / 2.0),
...
if __name__ == "__main__":
    ...
    train_until(max_iteration, data_sources, ribo_sources, input_shape, output_shape,
                dt_scaling_factor, loss_name, labels, net_name)
