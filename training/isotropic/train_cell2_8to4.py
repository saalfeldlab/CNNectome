...
def train_until(max_iteration, data_sources, input_shape, output_shape, dt_scaling_factor, loss_name, labels):
    ...
    voxel_size_raw = Coordinate((8, 8, 8))
    voxel_size_label = Coordinate((2, 2, 2))
    voxel_size_output = Coordinate((4, 4, 4))
    input_size = Coordinate(input_shape) * voxel_size_raw
    #label_size = Coordinate(output_shape)* voxel_size_label
    ...
    for label in labels:
        ...
        outputs_spec[label.pred_dist_key] = ArraySpec(voxel_size=voxel_size_output)
    ...
    request.add(ArrayKeys.RAW, input_size, voxel_size=voxel_size_raw)
    request.add(ArrayKeys.GT_LABELS, output_size,  voxel_size=voxel_size_label)
    request.add(ArrayKeys.MASK_UP, output_size, voxel_size=voxel_size_label)
    ...
    train_pipeline = (
        data_sources +
        RandomProvider() +
        #ElasticAugment((200, 200, 200), (10., 10., 10.), (0, math.pi/2.0),
...
if __name__ == "__main__":
    ...
    data_sources = ['gt_cell2_v1.1', ]
    input_shape = (196, 196, 196)
    output_shape = (308, 308, 308)
