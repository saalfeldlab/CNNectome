...
from gunpowder.contrib import ZeroOutConstSections, AddDistance, AddPrePostCleftDistance, TanhSaturate
import gpn
...
def make_cleft_to_prepostsyn_neuron_id_dict(csv_files, filter_comments):
    ...
    for csv_f in csv_files:
        ...
        for row in reader:
            if int(row['cleft']) > 0:
                if row['pre_comment'] not in filter_comments and row['post_comment'] not in filter_comments:
...
def train_until(max_iteration, samples, aug_mode, input_shape, output_shape, cremi_dir, n5_filename_format,
                csv_filename_format, filter_comments, dt_scaling_factor, loss_name,net_name='unet_train',
    ...
    csv_files = [os.path.join(cremi_dir, csv_filename_format.format(sample)) for sample in samples]
    cleft_to_pre, cleft_to_post = make_cleft_to_prepostsyn_neuron_id_dict(csv_files, filter_comments)
    ...
    data_sources = tuple(
        ...
        RandomLocationWithIntegralMask(integral_mask=ArrayKeys.INTEGRAL_MASK, min_masked=keep_thr) +
    ...
    if aug_mode == 'deluxe':
        train_pipeline = (
            ...
            SimpleAugment(transpose_only=[1, 2], mirror_only=[1, 2]) +
            gpn.Misalign(40, prob_slip=0.05, prob_shift=0.05, max_misalign=(10,10),
                         ignore_keys_for_slip=(ArrayKeys.GT_CLEFTS, ArrayKeys.CLEFT_MASK, ArrayKeys.PRE_MASK,
                                               ArrayKeys.POST_MASK, ArrayKeys.TRAINING_MASK, ArrayKeys.GT_LABELS)) +
            IntensityAugment(ArrayKeys.RAW, 0.9, 1.1, -0.1, 0.1, z_section_wise=True) +
        ...
        train_pipeline = (
            train_pipeline +
            gpn.ElasticAugment((40, 4, 4), (4, 40, 40), (0., 0., 0.), (0, math.pi / 2.0),
                               spatial_dims=3, subsample=8) +
