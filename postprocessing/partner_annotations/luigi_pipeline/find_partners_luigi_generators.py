...
class Matchmaker(object):
    def __init__(self, syn_file, cleft_cc_ds, pre_ds, post_ds, seg_file, seg_ds, tgt_file, raw_file=None, raw_ds=None,
        ...
        if raw_file is not None:
            ...
            self.rawf = None
            self.raw = None
    def list_of_clefts(self):
        for cid in self.list_of_cleftids:
            yield Cleft(self, cid, safe_mem = self.safe_mem, pre_thr=self.pre_thr, post_thr=self.post_thr,
                        dist_thr=self.dist_thr, size_thr=self.size_thr)
