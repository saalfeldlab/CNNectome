...
def single_gpu_inference(data_train, augmentation, data_eval, samples, gpu, iteration):
    path = '/nrs/saalfeld/heinrichl/synapses/data_and_augmentations/{0:}/{1:}'.format(data_train, augmentation)
    ...
    output_key = [net_io_names["pre_dist"], net_io_names["post_dist"], net_io_names["cleft_dist"]]
    ...
    for k, de in enumerate(data_eval):
        for s in samples:
            ...
            raw_file = '/groups/saalfeld/saalfeldlab/larissa/data/cremieval/{0:}/{1:}.n5'.format(de, s)
            out_file = '/nrs/saalfeld/heinrichl/synapses/data_and_augmentations/{0:}/{' \
                       '1:}/evaluation/{2:}/{3:}/{4:}.n5'.format(data_train, augmentation, iteration, de, s)
            ...
            run_inference_n5(prediction,
                             ...
                             log_processed=os.path.join(out_file, 'list_gpu_{0:}processed.txt'.format(gpu)))
