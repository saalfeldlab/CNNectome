...
class SynapticRegion(object):
    ...
    def erode_seg_mask(self):
        ...
        for k in range(int(self.erosion_steps / 10.)):
            self.segmask_eroded = scipy.ndimage.morphology.binary_erosion(self.segmask_eroded, structure=xy_structure,
                                                                          iterations=10)
            self.segmask_eroded = scipy.ndimage.morphology.binary_erosion(self.segmask_eroded, structure=z_structure,
                                                                          iterations=1)
        if self.erosion_steps % 10 != 0:
            self.segmask_eroded = scipy.ndimage.morphology.binary_erosion(self.segmask_eroded,
                                                                          structure=xy_structure, iterations=
                                                                          self.erosion_steps % 10)
    ...
    def make_region_for_point(self):
        if self.intersect_with_dilated_cleft_mask:
            self.region_for_point = np.logical_and(self.get_segmask_eroded(), self.cleft.get_dilated_cleft_mask())
...
class Cleft(object):
    def __init__(self, matchmaker, cleft_id, dilation_steps=7, safe_mem=False, size_thr=50):
        self.mm = matchmaker
        ...
        bbox = [bb + shift for bb, shift in zip(bbox, [-(3 * dilation_steps) // 10, 1 + (3 * dilation_steps) // 10,
                                                       -3 * dilation_steps,
                                                       3 * dilation_steps + 1, -3 * dilation_steps,
                                                       3 * dilation_steps + 1])]
        ...
        self.synregions = [SynapticRegion(segid, self) for segid in self.find_segments()]
...
class Matchmaker(object):
    def __init__(self, syn_file, cleft_cc_ds, seg_file, seg_ds, tgt_file, raw_file=None, raw_ds=None,
                 offset=(0., 0., 0.), num_cores=10, safe_mem=False):
        ...
        self.list_of_clefts = [Cleft(self, cid, safe_mem=safe_mem) for cid in self.list_of_cleftids]
    ...
    def prepare_file(self):
        ...
        self.cremi_file.write_clefts(cremi.Volume(self.cleft_cc[:], resolution=(40., 4., 4.),
                                                  offset=self.offset))
    ...
    def extract_dat(self, distances_filename, presizes_filename, postsizes_filename, sizes_filename):
        distances = []
        presizes = []
        postsizes = []
        sizes = []
...
def main(samples):
    ...
    for sample in samples:
        ...
        if '+' in sample:
            ...
            raw_file = '/groups/saalfeld/saalfeldlab/larissa/data/cremi-2017/sample_{' \
        ...
        print("initializing Matchmaker for sample {0:}".format(sample))
        mm = Matchmaker(syn_file, cleft_cc_ds, seg_file, seg_ds, filename_tgt, raw_file, raw_ds,
                        offsets[sample], safe_mem=True)
        print("preparing file for sample {0:}".format(sample))
        mm.prepare_file()
        print("finding partners for sample {0:}".format(sample))
        mm.write_partners()
