...
def cc2(filename_src, dataset_src_high_thr, dataset_src_low_thr, filename_tgt, dataset_tgt):

    srcf = z5py.File(filename_src, use_zarr_format=False)
    if not os.path.exists(filename_tgt):
        os.makedirs(filename_tgt)
    assert srcf[dataset_src_high_thr].attrs['offset'] == srcf[dataset_src_low_thr].attrs['offset']
    ...
    for sample in samples:
        ...
        for thrs in thrs_mult:
            for ds_src, ds_tgt in zip(dataset_srcs, dataset_tgts):
                ...
                cc2(filename_src.format(sample), ds_src.format(thrs[0]), ds_src.format(thrs[1]), filename_tgt.format(
