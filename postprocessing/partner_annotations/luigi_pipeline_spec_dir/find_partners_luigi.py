...
class SynapticRegion(object):
    def __init__(self, segmentid, parentcleft, region, pre_thr=35, post_thr=35, size_thr=5, dist_thr=600):
...
class FindPartners(luigi.Task):
    ...
    def requires(self):
        return ConnectedComponents(self.it, self.path, self.de, self.samples, self.data_eval)

    def output(self):
        ret = []
        for s in self.samples:
            ret.append(luigi.LocalTarget(os.path.join(os.path.dirname(self.input().fn),  'partners{0:}.msg'.format(
                s))))
        return ret
