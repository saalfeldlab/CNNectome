...
class SynapticRegion(object):
    ...
    def partner_with_post(self, partner):
        ...
        if not self.is_neighbor(partner):
            ...
            return False
        post_spot = np.round(
            scipy.ndimage.center_of_mass(partner.get_region_for_acc())
        ).astype(np.int)
        print(partner.cleft.get_seg().shape, post_spot)
        if partner.cleft.get_seg()[tuple(post_spot)] != partner.segmentid:
            print(
                "COM is not in segment - cleftid {0:}, segmentid {1:}".format(
                    partner.cleft.cleft_id, partner.segmentid
                )
            )
            return False
        gradient = []
        ...
        for ax, gr in enumerate(partner.cleft.get_cleft_gradient()):
            grad = np.ma.mean(
                np.ma.array(gr, mask=np.logical_not(partner.region_for_acc))
            )
            if grad > 0:
                max_lambda = (
                    partner.cleft.get_cleft().shape[ax] - 1 - post_spot[ax]
                ) / grad
            else:
        ...
        z, y, x = (
            np.linspace(post_spot[0], end_of_line[0], 100),
            np.linspace(post_spot[1], end_of_line[1], 100),
            np.linspace(post_spot[2], end_of_line[2], 100),
        )
        cleft_line = scipy.ndimage.map_coordinates(
            partner.cleft.get_cleft(), np.vstack((z, y, x))
        )
        seg_line = [
            self.cleft.get_seg()[int(round(z[k])), int(round(y[k])), int(round(x[k]))]
            for k in range(100)
        ]
        try:
...
class FindPartners(luigi.Task):
    ...
    def requires(self):
        return ConnectedComponents(
            self.it, self.path, self.de, self.samples, self.data_eval
        )
    ...
    def output(self):
        ...
        for s in self.samples:
            ret.append(
                luigi.LocalTarget(
                    os.path.join(
                        os.path.dirname(self.input().fn), "partners{0:}.msg".format(s)
                    )
                )
            )
        return ret
