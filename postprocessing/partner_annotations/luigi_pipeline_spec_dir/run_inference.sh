...
export NAME=$(basename $PWD-prediction-$4-$5)
export USER_ID=${UID}
GUNPOWDER_PATH=$(readlink -f $HOME/Projects/git-repos/gunpowder)
SIMPLEFERENCE_PATH=$(readlink -f $HOME/Projects/simpleference)
PRED_PATH=$(readlink -f $HOME/Projects/CNNectome/postprocessing/partner_annotations_single_luigi/)
...
nvidia-docker run --rm \
    ...
    /bin/bash -c "export CUDA_VISIBLE_DEVICES=$4;
    export PYTHONPATH=${GUNPOWDER_PATH}:${SIMPLEFERENCE_PATH}:${Z_PATH}:\$PYTHONPATH;
    python -u ${PRED_PATH}/run_prediction.py $1 $2 $3 $4 $5"
