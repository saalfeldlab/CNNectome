...
class SingleEvaluation(luigi.WrapperTask):
    ...
    path = luigi.Parameter(default='/nrs/saalfeld/heinrichl/synapses/pre_and_post/pre_and_post-v9.0/run01/')
    ...
    def requires(self):
        for de in self.data_eval:
            if 'A+' in self.samples or 'B+' in self.samples or 'C+' in self.samples:
                ...
                yield SplitModi(self.iteration, self.path, de, 'groundtruth', test_samples, self.data_eval)
            if 'A' in self.samples or 'B' in self.samples or 'C' in self.samples:
                training_samples = []
                for s in self.samples:
                    if not ('+' in s):
                ...
                yield PartnerReport(self.iteration, self.path, de, 'groundtruth', training_samples, self.data_eval)
                yield PartnerReport(self.iteration, self.path, de, 'validation', training_samples, self.data_eval)
                yield PartnerReport(self.iteration, self.path, de, 'training', training_samples, self.data_eval)
