...
def crop_main(it):
    ...
    dataset_srcs = ['predictions_it{0:}/cleft_dist'.format(it),
    ...
    dataset_tgts = ['predictions_it{0:}/cleft_dist_cropped'.format(it),
    ...
    for sample in samples:
        ...
        for ds_src, ds_tgt in zip(dataset_srcs, dataset_tgts):
            ...
            crop.crop_to_seg(filename_src.format(sample), ds_src, filename_tgt.format(sample), ds_tgt, off, sh)
...
def cc_main(it):
    ...
    filename_tgt = '/nrs/saalfeld/heinrichl/synapses/pre_and_post/pre_and_post-v6.3/cremi/{0:}.n5'
    dataset_tgts = ['predictions_it{0:}'.format(it)+'/cleft_dist_cropped_thr{0:}_cc{1:}']
    ...
    for sample in samples:
        ...
        for thrs in thrs_mult:
            for ds_src, ds_tgt in zip(dataset_srcs, dataset_tgts):
                ...
                connected_components_double_threshold.cc2(filename_src.format(sample), ds_src.format(thrs[0]),
                                                          ds_src.format(thrs[1]),
                        filename_tgt.format(
                    sample), ds_tgt.format(*thrs))
